...
document.addEventListener("DOMContentLoaded", () => {
    cargarFrasesPorCategoriaNavidad();
    configurarBarraBusquedaNavidad();
});
const categoriaSeleccionada = "Navidad";


function configurarBarraBusquedaNavidad() {
    const listaFrasesCat = document.getElementById("lista-frases-cat-nav"); // Assuming "cat" in ID refers to category
...
function cargarFrasesPorCategoriaNavidad() {
    fetch('frases.json')
        .then(response => response.json())
        .then(data => {
            const listaFrases = document.getElementById("lista-frases-cat-nav");
            listaFrases.innerHTML = '';

            const navidadFrases = data.frases.filter(fraseObj =>
                fraseObj.categorias.some(categoria => categoria.toLowerCase().includes("navidad"))
            );
